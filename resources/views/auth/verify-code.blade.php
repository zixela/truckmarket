...
<div class="mx-auto max-w-md rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6">
    <h1 class="mb-2 text-xl font-bold">{{ __('auth.verify_title') }}</h1>
    <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">{{ __('auth.verify_intro', ['email' => auth()->user()->email]) }}</p>
    ...
    <form method="POST" action="{{ route('verification.resend') }}" class="mt-3 text-center">
        @csrf
        <button type="submit" class="text-sm text-brand-600 dark:text-brand-400 hover:underline">{{ __('auth.verify_resend') }}</button>
    </form>
</div>
@endsection
