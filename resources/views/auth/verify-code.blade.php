...
<div class="mx-auto max-w-md rounded-lg border border-gray-200 bg-white p-6">
    ...
    <p class="mb-4 text-sm text-gray-600">{{ __('auth.verify_intro', ['email' => auth()->user()->email]) }}</p>
    ...
    <form method="POST" action="{{ route('verification.resend') }}" class="mt-3 text-center">
        @csrf
        <button type="submit" class="text-sm text-brand-600 hover:underline">{{ __('auth.verify_resend') }}</button>
    </form>
</div>
@endsection
