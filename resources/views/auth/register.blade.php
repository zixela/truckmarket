...
<div class="mx-auto max-w-md rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6">
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4"
        ...
        <label class="block text-sm">
            ...
            <input type="password" name="password_confirmation" required class="w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2">
        </label>
    ...
    <p class="mt-4 text-center text-sm text-gray-600 dark:text-gray-400">
        {{ __('auth.have_account') }} <a href="{{ route('login') }}" class="text-brand-600 dark:text-brand-400 hover:underline">{{ __('common.login') }}</a>
    </p>
</div>
@endsection
