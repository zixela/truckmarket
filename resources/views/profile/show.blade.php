...
<div class="grid gap-6 lg:grid-cols-[300px_1fr]">
    ...
    <aside class="space-y-4 rounded-lg border border-gray-200 bg-white p-4 text-center">
        @if ($user->avatarUrl())
            <img src="{{ $user->avatarUrl() }}" alt="{{ $user->name }}" class="mx-auto h-24 w-24 rounded-full object-cover">
        @else
            <div class="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gray-100 text-3xl">👤</div>
        @endif
        ...
        <div>
            ...
            <div class="text-sm text-gray-600">{{ $rating['average'] }} • {{ $rating['count'] }} {{ __('account.reviews') }}</div>
            <div class="text-xs text-gray-500">👍 {{ $rating['positive'] }} • 👎 {{ $rating['negative'] }}</div>
        </div>
    </aside>
            ...
            @forelse ($reviews as $review)
                <div class="mb-3 rounded-lg border border-gray-200 bg-white p-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="rounded-full px-2 py-0.5 text-xs {{ $review->is_positive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            {{ $review->is_positive ? __('account.review_positive') : __('account.review_negative') }}
                        </span>
                    </div>
                    <div class="text-sm text-gray-500"><x-stars :score="$review->score" /> • {{ $review->created_at->format('M d, Y') }}</div>
                    <p class="mt-2 text-sm text-gray-700">{{ $review->body }}</p>
                    @if ($review->reply)
                        <div class="mt-2 border-l-2 border-brand-300 pl-3 text-sm italic text-gray-600">{{ $review->reply }}</div>
                    @endif
                </div>
            @empty
                <div class="rounded-lg border border-dashed border-gray-300 bg-white p-6 text-center text-gray-500">
