...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data"
    ...
    <label class="block text-sm">
        ...
        <select name="type" x-model="type" @disabled($editing) class="w-full rounded-md border border-gray-300 px-3 py-2 disabled:bg-gray-100">
            @foreach ($types as $t)
    ...
    <div x-show="type === 'truck'" x-cloak class="space-y-4">
        <div class="grid gap-4 sm:grid-cols-2">
            <label class="block text-sm">
                ...
                <select name="deal" class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'truck'">
            ...
            <label class="block text-sm">
                ...
                <input type="text" name="make_model" value="{{ $old('make_model', $detail?->make_model) }}" placeholder="Freightliner Cascadia"
                       class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'truck'">
            </label>
        </div>
        <div class="grid gap-4 sm:grid-cols-3">
            <label class="block text-sm">
                <span class="mb-1 block font-medium">{{ __('listings.fields.cab_type') }}</span>
                <select name="cab_type" class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'truck'">
                    <option value="">—</option>
    ...
    <div x-show="type === 'trailer'" x-cloak class="space-y-4">
        <div class="grid gap-4 sm:grid-cols-2">
            <label class="block text-sm">
                <span class="mb-1 block font-medium">{{ __('listings.fields.deal') }}</span>
                <select name="deal" class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'trailer'">
                    <option value="sell" @selected($old('deal', $detail?->deal?->value) === 'sell')>{{ __('listings.deals.sell') }}</option>
                    <option value="rent" @selected($old('deal', $detail?->deal?->value) === 'rent')>{{ __('listings.deals.rent') }}</option>
                </select>
            </label>
            <label class="block text-sm">
                ...
                <select name="trailer_type" class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'trailer'">
    ...
    <div x-show="type === 'load'" x-cloak class="space-y-4">
        <div class="grid gap-4 sm:grid-cols-3">
            <label class="block text-sm">
                ...
                <select name="load_type" class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'load'">
            ...
            <label class="block text-sm">
                ...
                <input type="text" name="pickup_zip" value="{{ $old('pickup_zip', $detail?->pickup_zip) }}" placeholder="10001"
                       class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'load'">
            </label>
            <label class="block text-sm">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <label class="block text-sm">
                ...
                <select name="vehicle_type" class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'load'">
                    <option value="">—</option>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <label class="block text-sm">
                ...
                <input type="text" name="company_name" value="{{ $old('company_name', $detail?->company_name) }}" placeholder="Vmoon Corporation"
                       class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'company'">
            </label>
            <label class="block text-sm">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <label class="block text-sm">
                ...
                <input type="number" name="experience_years" min="0" max="60" value="{{ $old('experience_years', $detail?->experience_years) }}"
                       class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'dispatcher'">
            </label>
            <label class="block text-sm">
                <span class="mb-1 block font-medium">{{ __('listings.fields.employment_type') }}</span>
                <select name="employment_type" class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'dispatcher'">
                    <option value="">—</option>
        ...
        <div class="text-sm">
            ...
            <div class="flex flex-wrap gap-2">
                @foreach (['english_a1', 'english_b2', 'english_c1', 'russian', 'georgian'] as $lang)
                    <label class="cursor-pointer rounded-full border border-gray-300 px-3 py-1 text-xs has-[:checked]:border-brand-500 has-[:checked]:bg-brand-50 has-[:checked]:text-brand-700">
    ...
    <div x-show="type === 'driver_owner'" x-cloak class="space-y-4">
        <div class="grid gap-4 sm:grid-cols-3">
            <label class="block text-sm">
                <span class="mb-1 block font-medium">{{ __('listings.fields.experience') }}</span>
                <input type="number" name="experience_years" min="0" max="60" value="{{ $old('experience_years', $detail?->experience_years) }}"
                       class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'driver_owner'">
            </label>
            <label class="block text-sm">
                <span class="mb-1 block font-medium">{{ __('listings.fields.cdl_class') }}</span>
                <select name="cdl_class" class="w-full rounded-md border border-gray-300 px-3 py-2" :disabled="type !== 'driver_owner'">
                    <option value="">—</option>
            ...
            <label class="mt-6 flex items-center gap-2 text-sm">
                <input type="checkbox" name="owns_truck" value="1" class="rounded border-gray-300" :disabled="type !== 'driver_owner'"
    ...
    <label class="block text-sm">
        ...
        <textarea name="description" rows="4" class="w-full rounded-md border border-gray-300 px-3 py-2">{{ $old('description', $listing?->description) }}</textarea>
    </label>
    ...
    <div class="text-sm">
        ...
        <input type="file" name="photos[]" accept="image/*" multiple
               class="w-full rounded-md border border-gray-300 px-3 py-2">
        <p class="mt-1 text-xs text-gray-500">{{ __('listings.photos_limit', ['count' => \App\Models\Listing::MAX_PHOTOS]) }}</p>
    </div>
            ...
            <div class="grid grid-cols-3 gap-3 sm:grid-cols-5">
                @foreach ($listing->getMedia(\App\Models\Listing::PHOTO_COLLECTION) as $photo)
                    <label class="relative block cursor-pointer">
                        ...
                        <span class="absolute right-1 top-1 rounded bg-white/90 px-1 text-xs">
    ...
    <div class="flex justify-end gap-3">
        <a href="{{ route('account.listings.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm hover:bg-gray-50">
            {{ __('common.cancel') }}
        </a>
