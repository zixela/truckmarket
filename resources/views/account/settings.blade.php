...
<form method="POST" action="{{ route('account.settings.update') }}" class="space-y-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
    @csrf
    @method('PUT')

    <div class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('account.profile_fields') }}</div>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        ...
        <label class="block text-sm">
            ...
            <select name="locale" class="w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2">
    ...
    <div class="border-t border-gray-100 dark:border-gray-800 pt-4 text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('account.new_password') }}</div>
    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('account.new_password_hint') }}</p>

    <div class="grid gap-4 sm:grid-cols-2">
        <label class="block text-sm">
            <span class="mb-1 block font-medium">{{ __('account.new_password') }}</span>
            <input type="password" name="password" class="w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2">
        </label>
        <label class="block text-sm">
            <span class="mb-1 block font-medium">{{ __('auth.password_confirm') }}</span>
            <input type="password" name="password_confirmation" class="w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2">
        </label>
    </div>

    <div class="border-t border-gray-100 dark:border-gray-800 pt-4 text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('account.notifications') }}</div>
    <label class="flex items-center gap-2 text-sm">
        <input type="checkbox" name="notify_by_email" value="1" class="rounded border-gray-300 dark:border-gray-600"
