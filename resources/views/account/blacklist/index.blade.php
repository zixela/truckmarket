...
<p class="text-sm text-gray-500">{{ __('account.blacklist_intro') }}</p>
...
<div class="flex flex-wrap items-start justify-between gap-3">
    <form method="GET" action="{{ route('account.blacklist.index') }}" class="flex gap-2">
        ...
        <button type="submit" class="rounded-md border border-gray-300 px-3 py-2 text-sm hover:bg-gray-50">{{ __('common.search') }}</button>
    </form>
...
@forelse ($entries as $entry)
    <div class="rounded-lg border border-gray-200 bg-white p-4">
        <div class="flex flex-wrap items-center justify-between gap-2">
            <div>
                <a href="{{ route('profile.show', $entry->blockedUser) }}" class="font-semibold text-brand-600 hover:underline">
        ...
        <div class="mt-1 text-xs text-gray-500">{{ __('account.added_on') }}: {{ $entry->created_at->format('M d, Y') }}</div>
        <div class="text-sm text-gray-700">{{ $entry->reason ?: __('account.no_reason') }}</div>
    </div>
@empty
